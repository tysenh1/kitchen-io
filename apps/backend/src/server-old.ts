...
const kitchenDb = new sqlite3.Database('./kitchen.db');
const itemDb = new sqlite3.Database('./item.db')
...
const toolsLogic: KitchenTools = {
  ...
  //     kitchenDb.all("SELECT item_name, quantity, unit FROM pantry WHERE quantity > 0;", (err, rows) => {
...
`;
      kitchenDb.get(query, [recipe_id], (err, row) => {
        console.log(err)
...
io.on('connection', (socket) => {
  ...
  socket.on('barcode', async (barcode) => {
    const localItem = itemDb.prepare('SELECT id FROM item WHERE id = ?').get(barcode)

    let itemName = localItem?.common_name;
